...
// چابی کو کوڈ میں ڈائریکٹ فکس کر دیا گیا ہے تاکہ ورسیل کنفیوز نہ ہو
            ...
            <script>
                async function sendMsg() {
                    ...
                    try {
                        ...
                        chat.innerHTML += '<div class="message bot-msg" style="color: red;">سستم کنکشن ایرر! لائن میں کچھ رکاوٹ ہے۔</div>';
                    }
                    chat.scrollTop = chat.scrollHeight;
                }
            </script>
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        if (data.candidates && data.candidates[0].content.parts[0].text) {
            ...
            res.json({ reply: "جی میرے سرتاج؟ میں سن رہی ہوں۔" });
        }
    } catch (error) {
        console.error(error);
        res.json({ reply: "معذرت میرے فرخ، سرور نیٹ ورک میں کوئی عارضی رکاوٹ ہے۔" });
    }
});
