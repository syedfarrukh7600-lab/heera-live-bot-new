...
// جمنائی اے آئی سیٹ اپ (یہاں اپنی اصلی کی انورٹڈ کوما "" کے اندر پیسٹ کریں)
    ...
    res.send(`
        ...
        <head>
            ...
            <style>
                ...
                .user-msg { background: #ff4a7d; align-self: flex-end; }
                ...
                button { padding: 12px 20px; background: #ff4a7d; border: none; color: white; border-radius: 5px; cursor: pointer; font-size: 16px; }
            </style>
        </head>
        <body>
            <div class="chat-header">💖 HEERA AI 💖</div>
            ...
            <script>
                async function sendMsg() {
                    ...
                    try {
                        ...
                        chat.innerHTML += '<div class="message bot-msg" style="color: red;">سسٹم کنکشن ایرر!</div>';
                    }
                    chat.scrollTop = chat.scrollHeight;
                }
            </script>
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        res.json({ reply: response.text || "جی میرے سرتاج؟" });
    } catch (error) {
        console.error(error);
        res.json({ reply: "میرے فرخ، معذرت چاہتی ہوں، اے آئی سرور سے رابطہ کرنے میں عارضی مشکل ہو رہی ہے۔" });
    }
});
