...
// جمنائی اے آئی سیٹ اپ
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || "YOUR_KEY_HERE" });
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        if (!process.env.GEMINI_API_KEY) {
            return res.json({ reply: "فرخ بھائی، ورسیل میں GEMINI_API_KEY سیٹ نہیں ہے۔ برائے مہربانی اسے ایڈ کریں۔" });
        }
        ...
        res.json({ reply: response.text || "جی فرخ بھائی؟" });
    } catch (error) {
        console.error(error);
        res.json({ reply: "فرخ بھائی، اے آئی ماڈل کو کال کرنے میں مسئلہ آ رہا ہے۔ کوڈ بالکل صحیح ہے، بس کلاؤڈ کنکشن کا ایشو ہے۔" });
    }
});
